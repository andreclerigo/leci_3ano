...
static void printUsage(char *cmd_name)
{
    printf("Sinopsis: %s [OPTIONS] supp-file\n"
           "  OPTIONS:\n"
           "  -n name     --- set volume name (default: \"sofs21_disk\")\n"
...
{
    const char *volname = "sofs21_disk";    /* volume name */
    ...
    try
    {
        ...
        if (!quiet)
            infoMsg("Installing a %u-blocks sofs21 file system in file \"%s\"\n", ntotal, argv[optind]);

        /* compute structural division of the disk */
        uint32_t dbtotal;         // total number of data blocks
        uint16_t itsize;          // number of blocks of inode table
        if (!quiet)
            infoMsg("  Computing disk structure...\n");
        soComputeDiskStructure(ntotal, itotal, itsize, dbtotal);
        if (!quiet)
            infoMsg("    (itsize: %u, itotal: %u, dbtotal: %u).\n", itsize, itsize * IPB, dbtotal);

        /* filling in the superblock fields: */
        if (!quiet)
            infoMsg("  Filling in the superblock...\n");
        soFillInSuperblock(volname, ntotal, itsize, dbtotal);

        /* filling in the inode table: */
        if (!quiet)
            infoMsg("  Filling in the inode table...\n");
        soFillInInodeTable(itsize, date);

        /* filling in the bitmap table */
        if (!quiet)
            infoMsg("  Filling in the bitmap table...\n");
        soFillInBitmapTable(ntotal, itsize, dbtotal);

        /* filling in the root directory: */
        if (!quiet)
            infoMsg("  Filling in the root directory...\n");
        soFillInRootDir(ntotal, itsize, dbtotal);
        ...
        if (zero)
        {
            ...
            soZeroFreeDataBlocks(ntotal, itsize, dbtotal);
        }
        ...
        soCloseRawDisk();
        if (!quiet)
            infoMsg("A %ld-inodes SOFS21 file system was successfully installed in %s.\n",
