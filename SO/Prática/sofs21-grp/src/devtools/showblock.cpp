 ...
 *  \ingroup tools
 *  \brief The \b sofs21 show block program.
 * 
 *  \details 
...
static void printUsage(char *cmd_name)
{
    printf("Sinopsis: %s [ OPTION ] supp-file\n"
           ...
           "  -b range   --- show block(s) as bitmap 32-words\n"
           "  -D         --- debug mode\n"
           "  -h         --- print this help\n", cmd_name);
}
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "x:a:s:i:d:r:b:Dh")) != -1)
    {
        switch (opt)
        {
            ...
            case 'b':        /* show block contents as bitmap 32-words */
            {
                if (sopt != '_')
                {
    ...
    for (uint32_t i = i1; i <= i2; i++, first_time = false)
    {
        ...
        switch (sopt)
        {
            ...
            case 'd':
                if (not first_time) printf("---\n");
                printBlockOfDirectorySlots(buf, off);
                off += DPB;
                break;
            case 'r':
                if (not first_time) printf("---\n");
